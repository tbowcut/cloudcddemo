...
class ModerationFormTest extends ModerationStateTestBase {
  ...
  protected function setUp() {
    ...
    $this->createContentTypeFromUI('Moderated content', 'moderated_content', TRUE, TRUE, [
      'draft',
      'needs_review',
      'published'
    ], 'draft');
  ...
  public function testModerationFormBlock() {
    ...
    $block = $this->drupalPlaceBlock('workbench_moderation_latest_block', [
      'region' => 'content',
    ]);

    // Save the block through the UI to add context information.
    $this->drupalGet('admin/structure/block/manage/' . $block->id());
    $this->drupalPostForm(NULL, [], 'Save block');

    // Disable moderation information display.
    $node_type = NodeType::load('moderated_content');
    $node_type->setThirdPartySetting('workbench_moderation', 'display_moderation_information', FALSE);
    $node_type->save();
    ...
    // The published view should show the moderation form and have "Published"
    // status, because it is the default revision.
    $this->drupalGet($canonical_path);
    $this->assertResponse(200);
    $this->assertText('Status', 'Form text found on the published view.');
    $this->assertText('Published', 'Correct status found on the published view.');
    ...
    $this->drupalGet($canonical_path);
    $this->assertResponse(200);
    $this->assertText('Status', 'Form text found on the node view page.');
    $this->assertText('Published', 'Correct status found on the node view page.');
