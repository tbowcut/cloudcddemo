...
abstract class ModerationStateTestBase extends WebTestBase {
   ...
   * @param bool $display_moderation_information
   *   TRUE if moderation information should be displayed
   * @param string[] $allowed_states
   *   Array of allowed state IDs
   * @param string $default_state
   *   Default state.
   */
  protected function createContentTypeFromUI($content_type_name, $content_type_id, $moderated = FALSE, $display_moderation_information = TRUE, array $allowed_states = [], $default_state = NULL) {
    ...
    if ($display_moderation_information) {
      $this->enableDisplayModerationInformationThroughUI($content_type_id, $allowed_states, $default_state);
    }
  }
  ...
  /**
   * Enable display moderation information for a specified content type, using
   * the UI.
   *
   * @param string $content_type_id
   *   Machine name.
   ...
   *   Default state.
   */
  protected function enableDisplayModerationInformationThroughUI($content_type_id, array $allowed_states, $default_state) {
    $this->drupalGet('admin/structure/types/manage/' . $content_type_id . '/moderation');
    $this->assertFieldByName('display_moderation_information');

    $edit['display_moderation_information'] = 1;
